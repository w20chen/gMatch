...
set_beginning_partial_matchings(
    ...
    for (int v : cg.cand[u0]) {
        int this_len = 0;
        int *this_set = cg.h_get_candidates(v, u1, this_len);
        for (int j = 0; j < this_len; j++) {
            h_beginning_partial_matching.emplace_back(v, this_set[j]);
        }
    }
...
BFS_Extend(
    ...
    int min_len = 0;
    int *min_set = cg.d_get_candidates(this_partial_matching[first_uu], cur_query_vertex, min_len);

    while (bn_mask) {
        char uu = __ffs(bn_mask) - 1;
        bn_mask &= ~(1u << uu);
        int this_len = 0;
        int *this_set = cg.d_get_candidates(this_partial_matching[uu], cur_query_vertex, this_len);
        if (this_len < min_len) {
            min_len = this_len;
            min_set = this_set;
        }
