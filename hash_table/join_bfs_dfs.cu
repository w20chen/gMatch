...
init_dfs_stacks(
    ...
    if (lane_id < UNROLL_MIN && partial_matching_id < partial_matching_cnt) {
        ...
        int min_len = 0;
        int min_set = cg.d_get_candidates_offset(first_vv, next_u, min_len);
        ...
        while (bn_mask) {
            ...
            int this_len = 0;
            int this_set = cg.d_get_candidates_offset(vv, next_u, this_len);
            if (this_len < min_len) {
                min_len = this_len;
                min_set = this_set;
            }
        ...
        while (*this_stk_len > begin_len) {
            ...
            if (flag) {
                ...
                while (bn_mask) {
                    ...
                    int this_len = 0;
                    int this_set = cg.d_get_candidates_offset(vv, this_u, this_len);

                    if (this_set == cand_ptr->cand_set[lane_parent_idx]) {
                        continue;
                    }

                    if (false == binary_search<int>(cg.d_cg_array + this_set, this_len, lane_v)) {
                        flag = false;
                        break;
                    }
                }
