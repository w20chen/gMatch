...
namespace STMatch {
  ...
  struct GraphPreprocessor {
    ...
    Graph* to_gpu() {
      ...
      std::cout << g.nnodes << ' ' << g.nedges << std::endl;

      cudaCheck(cudaMalloc(&gcopy.vertex_label, sizeof(bitarray32) * (ull)g.nnodes));
      printf("%lf MB\n", sizeof(bitarray32) * (ull)g.nnodes / 1024. / 1024.);
      check_gpu_memory();
      cudaCheck(cudaMalloc(&gcopy.rowptr, sizeof(graph_edge_t) * ((ull)g.nnodes + 1)));
      printf("%lf MB\n", sizeof(graph_edge_t) * ((ull)g.nnodes + 1) / 1024. / 1024.);
      check_gpu_memory();
      cudaCheck(cudaMalloc(&gcopy.colidx, sizeof(graph_node_t) * g.nedges));
      printf("%lf MB\n", sizeof(graph_node_t) * g.nedges / 1024. / 1024.);
      check_gpu_memory();
      printf("Need %lf MB\n", sizeof(graph_node_t) * g.nedges / 1024. / 1024.);
      cudaCheck(cudaMalloc(&gcopy.src_vtx, sizeof(graph_node_t) * g.nedges));
      check_gpu_memory();
      cudaMemcpy(gcopy.vertex_label, g.vertex_label, sizeof(bitarray32) * (ull)g.nnodes, cudaMemcpyHostToDevice);
      cudaMemcpy(gcopy.rowptr, g.rowptr, sizeof(graph_edge_t) * ((ull)g.nnodes + 1), cudaMemcpyHostToDevice);
    ...
    read_csr_file(std::string &filename) {
        ...
        for (int i = 0; i < g.nnodes; i++) {
            g.vertex_label[i] = vertex_label_[i];
        }
    ...
    {
      std::cout << (ull)g.nedges << std::endl;
