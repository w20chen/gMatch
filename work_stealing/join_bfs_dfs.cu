...
dfs_kernel(
    ...
    while (true) {
        ...
        if (this_offset >= partial_matching_cnt) {
            if (lane_id == 0) {
                atomicSub(begin_offset, UNROLL_MIN);
            }
            break;
        }
...
restart:
            ...
            stk_elem_cand *cand_ptr = &this_stk_elem[this_u].cand;

#ifdef LOCAL_WORK_STEALING
            // if there are idle warps within the thread block
            if (warpsPerBlock > unfinished) {
                for (int depth = begin_len; depth <= begin_len + 3 && depth <= Q.vcount() - 1; depth++) {
                    // if this level is okay to split
                    if (this_stk_elem[depth].start_set_idx != -1 &&
                            this_stk_elem[depth].unroll_size - this_stk_elem[depth].start_set_idx >= 2) {
                        if (lane_id == 0) {
                            int half_size = (this_stk_elem[depth].unroll_size - this_stk_elem[depth].start_set_idx) / 2;
                            int requested_id = 0;
                            ...
                            if (flag) {
                                ...
                                that_stk_elem[depth].start_set_idx = that_stk_elem[depth].unroll_size - half_size;
                                that_stk_elem[depth].start_idx_within_set = 0;
                                this_stk_elem[depth].unroll_size -= half_size;
            ...
            if (flag_mask == 0) {
                ...
                if (*this_stk_len == Q.vcount()) {
                    ...
                    stk_elem_cand *new_cand_ptr = &this_stk_elem[next_u].cand;
...
join_bfs_dfs(
...
#ifdef BALANCE_CNT
    cudaCheck(cudaMemcpyToSymbol(steal_cnt, &Zero_ull, sizeof(ull)));
    cudaCheck(cudaMemcpyToSymbol(first_finish, &Zero_ull, sizeof(ull)));
#endif
    ...
    int l = 2;
    for (; partial_matching_cnt < expected_init_num && l <= Q.vcount() - 3; l++) {
...
bfs_end:
    ...
    //     if (h_block_time[i] > 0) {
    //         printf("[%04d]: ", i);
    //         for (int j = 0; j < warpsPerBlock; j++) {
    //             printf("%llu ", h_warp_time[i * warpsPerBlock + j]);
    //         }
    //         if (i % 2 == 1) {
    //             printf("\n");
    //         }
...
#ifdef BALANCE_CNT
    ull h_steal_cnt = 0;
    cudaCheck(cudaMemcpyFromSymbol(&h_steal_cnt, steal_cnt, sizeof(ull)));
    printf("Work stealing count: %lld\n", h_steal_cnt);

    ull h_first_finish = 0;
    cudaCheck(cudaMemcpyFromSymbol(&h_first_finish, first_finish, sizeof(ull)));
    printf("First finished warp at %llu\n", h_first_finish);
#endif
