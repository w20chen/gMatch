...
def check(binary_path, data_graph_path, query_graph_path, initial):
    execution_args = generate_args(binary_path, '-d', data_graph_path, '-q', query_graph_path, '--initial', initial)
    ...
    if rc == 0:
        ...
        for line in std_output_list:
            ...
            if 'Idle Rate' in line:
                idle_rate = float(line.split(':')[-1].strip()[:-1])
        query_id = os.path.basename(query_graph_path).split('_')[-1]
        print(f'{query_id},{embedding_num},{elapsed_time},{idle_rate}')
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--initial', required=False, type=str, default='1000000', help='')
    args = parser.parse_args()

    binary_path = './bitmap/build/SubgraphMatching'
    ...
    for file_path in filenames:
        check(binary_path, data_path, file_path, args.initial)