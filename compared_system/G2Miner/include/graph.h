...
private:
  // 新增方法声明
  void read_csr_binary_format(const std::string& filename, bool use_vlabel, bool use_elabel, 
                             bool directed, bool need_reverse);
  void read_original_format(const std::string& prefix, bool use_dag, bool directed,
                           bool use_vlabel, bool use_elabel, bool need_reverse, bool bipartite);
  void convert_csr_to_internal(const std::vector<uint64_t>& offsets,
