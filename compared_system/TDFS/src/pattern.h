...
namespace STMatch {
    ...
    inline std::string
    GetCondOperatorString(const CondOperator &op) {
        std::string ret = "";
        switch (op) {
            case LESS_THAN:
                ret = "LESS_THAN";
    ...
    struct Pattern {
        ...
        int label_(int v) const {
            return vertex_labels[v];
        }

        int degree_(int v) const {
            return degree[v];
        }
    };
    ...
    struct PatternPreprocessor {
        ...
        PatternPreprocessor(std::string filename) {
            printf("Step 1: Reading pattern file...\n");
            readfile(filename);
            printf("Step 2: Setting conditions and getting bliss graph...\n");
            SetConditions(GetConditions(GetBlissGraph())); // set order_
            printf("Step 3: Getting matching order...\n");
            get_matching_order();
            printf("Step 4: Getting partial order...\n");
            get_partial_order();
            printf("Step 5: Getting set operations...\n");
            get_set_ops();
            printf("Step 6: Propagating partial order...\n");
            propagate_partial_order();
            printf("Step 7: Getting labels...\n");
            get_labels();
            printf("Step 8: Converting to one-dimensional format...\n");
            convert2oned();
            printf("Pattern preprocessing finished.\n");
        }
        ...
        std::string
        OrderToString(const std::vector<int> &p) {
            std::string res;
            for (auto v : p) {
                res += std::to_string(v);
            }
        ...
        void
        CompleteAutomorphisms(std::vector<std::vector<int>> &perm_group) {
        ...
        GetAutomorphisms(bliss::Graph *bg) {
        ...
        GetAEquivalenceClasses(const std::vector<std::vector<int>> &aut) {
        ...
        void
        SetConditions(const std::vector<std::pair<int, int>> &conditions) {
        ...
        restriction_generation(std::vector<uint32_t> &partial_order) const {
            assert(pat.nnodes <= 32);

            std::vector<std::vector<uint32_t>> automorphisms;
            int alpha = find_automorphisms_naive(automorphisms);
            ...
            // Compute R^2
            for (int u = 0; u < pat.nnodes; u++) {
                for (int v = 0; v < pat.nnodes; v++) {
                    M[u][v] = 0;
                    for (int w = 0; w < pat.nnodes; w++) {
            ...
            partial_order.resize(pat.nnodes, 0);
        ...
        bool is_adjacent_(int u, int v) const {
            return adj_matrix_[u][v] | adj_matrix_[v][u];
        }

        int
        find_automorphisms_naive(std::vector<std::vector<uint32_t>> &embeddings) const {
            // Note that this method is working on small graphs (tens of vertices) only.

            // This method conducts subgraph matching from Q to Q.
            std::vector<bool> visited(pat.nnodes, false);
            std::vector<uint32_t> idx(pat.nnodes);
            std::vector<uint32_t> mapping(pat.nnodes);
            std::vector<std::vector<uint32_t>> local_candidates(pat.nnodes);
            std::vector<std::vector<uint32_t>> global_candidates(pat.nnodes);
            std::vector<std::vector<uint32_t>> backward_neighbors(pat.nnodes);

            // Initialize global candidates.
            for (uint32_t u = 0; u < pat.nnodes; u++) {
                uint32_t u_label = pat.label_(u);
                uint32_t u_degree = pat.degree_(u);

                for (uint32_t v = 0; v < pat.nnodes; v++) {
                    uint32_t v_label = pat.label_(v);
                    uint32_t v_degree = pat.degree_(v);
            ...
            for (uint32_t i = 1; i < pat.nnodes; i++) {
                            ...
                            for (auto temp_v : global_candidates[u]) {
                                if (!visited[temp_v]) {
                                    ...
                                    for (auto uu : backward_neighbors[u]) {
                                        ...
                                        if (!is_adjacent_(temp_v, temp_vv)) {
                                            is_feasible = false;
                                            continue;
                                        }
                                    }
