...
static const char *asString(MAINTASK mt)
{
    ...
    case KCLIQUE:
        return "kc -> kclique";
    case KCLIQUE_LOCAL:
        return "lkc -> kclique-local";
    case MAXIMAL_CLIQUE:
        return "mc -> maximal-clique";
    case CROSSDECOMP:
        return "cd";
    case GRAPH_MATCH:
        return "sgm";
...
static KcliqueConfig parseKcConfig(const char *s)
{
    ...
    printf("> KC Config: Algo = %s, PartSize = %d, BinaryEncode = %s, EncodeHalf = %s\n",
           (kc.Algo == KCAlgoEnum::GraphOrient) ? "GraphOrient" : "Pivoting",
           kc.PartSize,
           kc.BinaryEncode ? "true" : "false",
           kc.EncodeHalf ? "true" : "false");
...
static Config parseArgs(int argc, char **argv)
{
    ...
    while ((opt = getopt(argc, argv, "g:r:d:m:x:o:a:k:h:v:s:p:e:q:w:t:c:n:")) >= 0)
    {
        ...
        case 'x':
            ...
            break;
        case 'o':
            config.orient = parseOrient(optarg);
            break;
        case 'a':
            config.allocation = parseAllocation(optarg);
            break;
        case 'v':
        ...
        case 's':
            ...
            break;
        case 'p':
            config.processBy = parseProcessBy(optarg);
            break;
        case 'e':
            config.processElement = parseElement(optarg);
            break;
        case 'q':
