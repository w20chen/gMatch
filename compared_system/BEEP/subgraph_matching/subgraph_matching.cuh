...
#pragma omp parallel for
                for (int d = first_d; d < first_d + ndev_; d++)
                {
                    ...
                    printf("> grid block size = %u\n", grid_block_size);
                    printf("> max deg = %u, num divs = %u, max_qDegree = %u\n", maxDeg, num_divs, max_qDegree);

                    uint64 encode_size = (uint64)grid_block_size * (uint64)maxDeg * (uint64)num_divs;
                    uint64 level_size = (uint64)grid_block_size * (uint64)max_qDegree * (uint64)num_divs * (uint64)numPartitions;
                    printf("> encode size: %lu, level size: %lu\n", encode_size, level_size);

                    GPUArray<uint64> dev_counter("Counter for each device", AllocationTypeEnum::gpu, sizeof(uint64), d);

                    printf("> trying to allocate %lu MB for node_be\n", (encode_size * sizeof(T)) / (1024 * 1024));
                    GPUArray<T> node_be("Temp level Counter", AllocationTypeEnum::gpu, encode_size, d);

                    printf("> trying to allocate %lu MB for current_level\n", (level_size * sizeof(T)) / (1024 * 1024));
                    GPUArray<T> current_level("Temp level Counter", AllocationTypeEnum::gpu, level_size, d);
