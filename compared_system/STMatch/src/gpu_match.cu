...
namespace STMatch {
    ...
    __device__ bool
    trans_layer(CallStack &_target_stk, CallStack &_cur_stk, Pattern *_pat, int _k, int ratio = 2) {
        ...
        _cur_stk.slot_storage[_pat->rowptr[0]][_target_stk.uiter[0]][_target_stk.iter[0] + _target_stk.uiter[1]] = _target_stk.slot_storage[_pat->rowptr[0]][_target_stk.uiter[0]][_target_stk.iter[0] +
                _target_stk.uiter[1]];
        _cur_stk.slot_storage[_pat->rowptr[0]][_target_stk.uiter[0]][_target_stk.iter[0] + _target_stk.uiter[1] + JOB_CHUNK_SIZE] =
            _target_stk.slot_storage[_pat->rowptr[0]][_target_stk.uiter[0]][_target_stk.iter[0] + _target_stk.uiter[1] + JOB_CHUNK_SIZE];

        for (int i = 1; i < _k; i++) {
            _cur_stk.slot_storage[_pat->rowptr[i]][_target_stk.uiter[i]][_target_stk.iter[i] + _target_stk.uiter[i + 1]] = _target_stk.slot_storage[_pat->rowptr[i]][_target_stk.uiter[i]][_target_stk.iter[i] +
                    _target_stk.uiter[i + 1]];
        }
    ...
    __device__ bool
    trans_skt(CallStack *_all_stk, CallStack *_cur_stk, Pattern *pat, StealingArgs *_stealing_args) {
    ...
    __forceinline__ __device__ graph_node_t
    path(CallStack *stk, Pattern *pat, int level, int k) {
        if (level > 0) {
    ...
    __forceinline__ __device__ graph_node_t *
    path_address(CallStack *stk, Pattern *pat, int level, int k) {
        if (level > 0) {
            return &(stk->slot_storage[pat->rowptr[level]][stk->uiter[level]][stk->iter[level] + k]);
        }
        else {
    ...
    bool
    bsearch_exist(DATA_T *set2, SIZE_T set2_size, DATA_T target) {
        if (set2_size <= 0) {
            return false;
        }
        int mid;
    ...
    __forceinline__ __device__
    void
    prefix_sum(int *_input, int input_size) {
        ...
        if (thid == 0) {
            _input[WARP_SIZE - 1] = 0;    // clear the last element
        }
    ...
    __forceinline__ __device__ void
    get_job(JobQueue *q, graph_node_t &cur_pos, graph_node_t &njobs) {
        lock(&(q->mutex));
        cur_pos = q->cur;
    ...
    __device__ void
    extend(Graph *g, Pattern *pat, CallStack *stk, JobQueue *q, pattern_node_t level) {
                ...
                if (!LABELED) {
                    ...
                    if (pat->partial[i] != 0) {
                        ...
                        for (pattern_node_t k = 1; k < level - 1; k++) {
                            if ((pat->partial[i] & (1 << (k + 1))) && ((i == pat->rowptr[level]) ^ (ub < path(stk, pat, k, stk->uiter[k + 1])))) {
                                ub = path(stk, pat, k, stk->uiter[k + 1]);
                            }
                        }
                        ...
                        for (pattern_node_t k = 0; k < arg[wid].num_sets; k++) {
                            ...
                            for (pattern_node_t j = 0; j < prev_level; j++) {
                                if ((pat->partial[i] & (1 << j)) && ((i == pat->rowptr[level]) ^ (arg[wid].ub[k] < path(stk, pat, j - 1, prev_iter)))) {
                                    arg[wid].ub[k] = path(stk, pat, j - 1, prev_iter);
                                }
                            }

                            if ((pat->partial[i] & (1 << level)) && ((i == pat->rowptr[level]) ^ (arg[wid].ub[k] < path(stk, pat, level - 1, k)))) {
                                arg[wid].ub[k] = path(stk, pat, level - 1, k);
                            }
                            if (arg[wid].ub[k] == -1) {
    ...
    __forceinline__ __device__ void
    respond_across_block(int level, CallStack *stk, Pattern *pat, StealingArgs *_stealing_args) {
    ...
    __global__ void
    _parallel_match(Graph *dev_graph, Pattern *dev_pattern,
